...
def _question_kb(sq: dict) -> InlineKeyboardMarkup:
    ...
    buttons = [
        InlineKeyboardButton(ds.letter_for(i), callback_data=f"dz_ans_{i}")
...
def _question_text(quiz: dict, sq: dict, index: int, remaining=None) -> str:
    ...
    options = sq.get("options", [])
    opt_lines = "\n".join(f"{ds.letter_for(i)}) {opt}" for i, opt in enumerate(options))
    return (
        f"{header}"
...
def _build_results_blocks(quiz: dict, score: int, total_points: int, correct_count: int,
    ...
    for i, r in enumerate(review, 1):
        ...
        opt_lines = [f"{ds.letter_for(j)}) {opt}" for j, opt in enumerate(options)]
        lines = [
...
async def handle_distro_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query   = update.callback_query
    user_id = query.from_user.id
    choice  = int(query.data.replace("dz_ans_", ""))  # 0-based option index
