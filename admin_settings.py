...
(AS_HUB, AS_ADD, AS_DEL_LIST, AS_DEL_CONFIRM) = range(4)
...
def _hub_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ إضافة مشرف", callback_data="as_add")],
...
async def as_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
...
def build_admin_settings_handler() -> ConversationHandler:
    ...
    return ConversationHandler(
        ...
        states={
            AS_HUB: [
                CallbackQueryHandler(as_add,      pattern="^as_add$"),
