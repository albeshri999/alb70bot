...
def _initiative_summary(initiative: dict) -> str:
    status = ins.initiative_status_label(initiative)
...
async def in_c_points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👥 كم العدد الأقصى للمشاركين المقبولين في هذه المبادرة؟\n\nمثال: 1 أو 2 أو 5 أو 10")
    return IN_C_MAX
...
def _edit_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("👥 الحد الأقصى للمشاركين", callback_data="in_e_max")],
        [InlineKeyboardButton("⛔ إغلاق/فتح المبادرة يدوياً", callback_data="in_e_toggle_closed")],
...
async def in_e_max(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await _reply(update, "👥 أرسل الحد الأقصى الجديد لعدد المشاركين المقبولين:")
    return IN_E_MAX
...
async def in_e_max_val(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.isdigit() or int(text) <= 0:
        ...
        return IN_E_MAX
    initiative_id = context.user_data.get("in_initiative_id")
    was_full = ins.is_full(ins.get_initiative(initiative_id))
    ins.update_initiative_field(initiative_id, max_participants=int(text))
    initiative = ins.get_initiative(initiative_id)
    await update.message.reply_text("✅ تم تحديث الحد الأقصى.", reply_markup=_edit_menu_kb())

    # If raising the cap reopened a previously-full initiative, let anyone
    # who's still pending know it's worth checking again isn't needed (they
    # were never auto-rejected) — nothing further to notify here; new
    # participants can simply request again since is_full() is now False.
    if was_full and not ins.is_full(initiative):
        logger.info("Initiative %s reopened after capacity increase.", initiative_id)
    return IN_EDIT_MENU


async def in_e_toggle_closed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    initiative_id = context.user_data.get("in_initiative_id")
    initiative = ins.get_initiative(initiative_id)
    ins.set_initiative_closed(initiative_id, not initiative.get("closed"))
    initiative = ins.get_initiative(initiative_id)
    label = "⛔ تم إغلاق المبادرة يدوياً." if initiative.get("closed") else "🔓 تم فتح المبادرة مجدداً."
    await _reply(update, label + "\n\n" + _initiative_summary(initiative), _edit_menu_kb())
    return IN_EDIT_MENU
...
async def _back_to_req_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def in_req_sel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not r or not initiative:
        return await _back_to_req_list(update, context)
...
async def in_req_accept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If this acceptance filled the last slot, let the OTHER still-pending
    # requesters for this same initiative know — their status stays
    # "pending" (the admin still decides each one individually).
    initiative = ins.get_initiative(initiative_id)
    if ins.is_full(initiative):
        for other in ins.requests_for_initiative(initiative_id, statuses=(ins.STATUS_PENDING,)):
            if str(other.get("user_id")) == str(user_id):
                continue
            try:
                await context.bot.send_message(
                    chat_id=int(other["user_id"]),
                    text=f"🔒 اكتمل العدد المطلوب لمبادرة: {initiative.get('name', '—')}.",
                )
            except Exception as e:
                logger.warning("initiative full-notice failed: %s", e)
    ...
    return ConversationHandler(
        ...
        states={
            ...
            IN_EDIT_MENU: [
                ...
                CallbackQueryHandler(in_e_toggle_closed, pattern="^in_e_toggle_closed$"),
                hub_reentry,
            ],
